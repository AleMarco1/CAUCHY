"""
CAUCHY — R5-1 Approccio 3, versione 3 (prior calibrato su n_gal Quijote)
src/phase_r51_hod_restricted_v3.py

Obiettivo: R5-1 pre-submission — marginalizzazione HOD con prior log_Mmin
calibrato sulla densità galattica osservata in B3 su simulazioni Quijote
a cosmologia centrale.

Storia versioni:
  v1: prior [12.2, 13.1] da letteratura — σ_marg=0.407σ (N=20, K=3, pilota)
  v2: prior [12.2, 13.1] + rejection n_gal∈[600K,1.2M] — σ_marg=0.191σ FAIL
      Diagnosi: prior troppo largo, VIF=0.801 (varianza HOD > varianza cosmo)
  v3: prior calibrato su n_gal Quijote (phase_r51_calibrate_mmin.py)
      log_Mmin* = 12.987 (produce n_gal~900K su cosmologia fiduciale Quijote)
      Prior v3: log_Mmin ∈ [12.837, 13.137] (±0.15 dex intorno a log_Mmin*)
      VIF atteso: non prevedibile analiticamente — misurato dal run

Motivazione prior v3 (anti-circolarità):
  log_Mmin* = 12.987 è derivato da phase_r51_calibrate_mmin.py:
  valore che produce n_gal~900K su 20 simulazioni Quijote a cosmologia
  centrale (|w₀+1|<0.08, Ωm∈[0.28,0.35]), con altri parametri HOD fissati
  ai valori B3. Non è derivato dal segnale TDA.
  Il prior ±0.15 dex incorpora: scatter inter-simulazione (~0.10 dex) +
  margine di sistematica (0.05 dex).

  Confronto range:
  Prior v2: [12.200, 13.100] = 0.900 dex
  Prior v3: [12.837, 13.137] = 0.300 dex  (riduzione 67%)

Gate R5-1 (invariato):
  PASS:       σ_marg ≥ 2.0σ
  BORDERLINE: 1.5 ≤ σ < 2.0σ
  FAIL:       σ_marg < 1.5σ
...
R51_CACHE_DIR    = RESULTS_DIR / "phase_r51_cache_v3"
OUTPUT_FEATURES  = RESULTS_DIR / "phase_r51_features_v3.npz"
OUTPUT_RESULTS   = RESULTS_DIR / "phase_r51_results_v3.json"
MANIFEST         = RESULTS_DIR / "phase_r51_manifest_v3.json"
