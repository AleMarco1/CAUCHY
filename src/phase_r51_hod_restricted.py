"""
CAUCHY — R5-1 Approccio 3, Opzione A (rejection sampling n_gal)
src/phase_r51_hod_restricted_v2.py

Obiettivo: R5-1 pre-submission — marginalizzazione HOD con prior ristretto
fisicamente motivato su DESI BGS r<19.5, per escludere HOD mis-calibration
come spiegazione alternativa dell'anomalia +3.09σ.

Design approvato dal PI (2026-06-08), revisione post-test-pilota (2026-06-08):
  - Prior: Zheng 2007 5 parametri, range fisicamente motivato dalla letteratura BGS
  - Q1: log_Mmin ∈ [12.2, 13.1]  (lower: vincolo fisico Phase 5; upper: Smith & Grove 2024 BGS)
  - Q2: Zheng 5-param (infrastruttura B3 riutilizzata, A_cen=A_sat=0 fissi)
  - N campi: 500, selezione uniforme su w₀ (seed=42)
  - K: 20 campioni HOD accettati per campo (forward Monte Carlo con rejection)
  - Threshold frozen: σ_marg ≥ 2.0σ → PASS (HOD mis-calibration esclusa)
  - Output diagnostico: VIF prior ristretto + convergenza in K (K=5,10,15,20)

Modifica v2 — Opzione A: rejection sampling su n_gal
  Il test pilota (N=20, K=3) ha mostrato n_gal_mean=1.1M vs target BGS ~900K.
  Causa: prior flat su log_Mmin → regioni basse di log_Mmin dominano n_gal
  (relazione fortemente non lineare).
  Soluzione: accetta θ_HOD^k solo se n_gal(θ_HOD^k, sim_i) ∈ [N_GAL_LO, N_GAL_HI].
  Questo corrisponde a un prior condizionato sull'osservabile n_gal, metodologicamente
  più corretto: condiziona sulla proprietà direttamente confrontabile con BGS.
  Parametri rejection (default): N_GAL_LO=600K, N_GAL_HI=1200K (±33% di 900K).
  Max tentativi per campione: MAX_ATTEMPTS=50 (se fallisce → fallback su HOD B3 mediano).
  K effettivo tracciato: frazione di accettazione per campo registrata nelle diagnostiche.

Prior Zheng 5-param — derivazione indipendente da B3:
  log_Mmin   ∈ [12.2, 13.1]  — Smith & Grove 2024 (BGS DR1) best-fit ~13.08 come upper ref;
                                 vincolo fisico CAUCHY Phase 5: log_Mmin < 12.2 produce
                                 popolazioni non rappresentative per BGS r<19.5 su Quijote.
  sigma_logM ∈ [0.2,  0.8]   — range fisico standard Zheng 2007 per survey otticamente selezionati
  log_M0     ∈ [11.5, 13.0]  — parametro soglia satellite, range conservativo
  log_M1     ∈ [13.0, 14.2]  — Smith & Grove 2024: ~13.8 come upper reference
  alpha      ∈ [0.5,  1.5]   — slope power-law satellite, range standard letteratura BGS

Nota anti-circolarità: i best-fit B3 (12.5, 0.55, 12.25, 13.5, 1.0) cadono dentro il
prior ma NON sono il suo centro: log_Mmin B3=12.5 è vicino al lower bound [12.2,13.1],
non al centro (12.65). Direzione causale: letteratura → prior → verifica compatibilità B3.

Metodologia forward sampling:
  E[f(θ_HOD)] ≈ (1/K) Σ_k f(θ_HOD^k), θ_HOD^k ~ π_ristretto(θ_HOD)
  Equivalente a integrazione Monte Carlo. Riferimento: SimBIG (Hahn+2023).
  Stessa giustificazione del Run A Phase 5 (CHANGELOG 2026-05-06), ora con prior calibrato.

Gate R5-1:
  PASS       σ_marg ≥ 2.0σ   → HOD mis-calibration esclusa come sistematica dominante
  BORDERLINE 1.5 ≤ σ < 2.0σ  → evidenza marginale, sistematica dichiarata ma non dominante
  FAIL       σ_marg < 1.5σ   → HOD mis-calibration sistematica dominante, claim rivisto

Traceabilità:
  - Derivato da: src/phase5_hod_b3.py (pipeline B3, bug v3 già corretti)
  - Prior source: Smith & Grove et al. 2024 (arXiv:2312.08792), vincolo fisico Phase 5
  - Output: results/phase_r51_results.json, results/phase_r51_features.npz

Uso:
  python src/phase_r51_hod_restricted.py [--n_sim 500] [--K 20] [--seed 42] [--resume]

Tempo stimato: ~26h (500 sim × 20 K × 9.5s/gudhi / 3600)
"""
...
R51_CACHE_DIR    = RESULTS_DIR / "phase_r51_cache_v2"
OUTPUT_FEATURES  = RESULTS_DIR / "phase_r51_features_v2.npz"
OUTPUT_RESULTS   = RESULTS_DIR / "phase_r51_results_v2.json"
MANIFEST         = RESULTS_DIR / "phase_r51_manifest_v2.json"
...
PRIOR_LOW_5  = np.array([12.2, 0.2, 11.5, 13.0, 0.5])
PRIOR_HIGH_5 = np.array([13.1, 0.8, 13.0, 14.2, 1.5])
...
# Verifica anti-circolarità: B3 deve cadere DENTRO il prior
assert np.all(B3_PARAMS_5 >= PRIOR_LOW_5) and np.all(B3_PARAMS_5 <= PRIOR_HIGH_5), \
    "ERRORE: B3 params fuori dal prior ristretto — verificare bounds"

print("=" * 70)
print("CAUCHY R5-1 — HOD prior ristretto fisicamente motivato (BGS r<19.5)")
print("=" * 70)
print(f"\nPrior Zheng 5-param (derivato da Smith & Grove 2024 + vincolo fisico Phase 5):")
...
for nm, lo, hi, b3 in zip(PRIOR_NAME_5, PRIOR_LOW_5, PRIOR_HIGH_5, B3_PARAMS_5):
    inside = "✓" if lo <= b3 <= hi else "✗ ERRORE"
    print(f"  {nm:<14} {lo:>8.3f} {hi:>8.3f} {b3:>8.3f}  {inside:>12}")
print(f"\nParametri fissi: A_cen=0, A_sat=0, eta_vel=1, eta_conc=1")
    ...
    if pos_h is None or len(pos_h) < 50:
        ...
        for k in range(args.K):
            ...
            for attempt in range(args.max_attempts):
                theta_5 = rng_hod.uniform(PRIOR_LOW_5, PRIOR_HIGH_5)
                hod_params_full = np.array([
                    theta_5[0], theta_5[1], theta_5[2], theta_5[3], theta_5[4],
...
results_out = {
    ...
    "prior_restricted": {
        ...
        "source":        "Smith & Grove 2024 (arXiv:2312.08792) + vincolo fisico Phase 5",
        ...
        "anti_circularity": "Prior derivato da letteratura indipendentemente da B3. "
                            "B3=(12.5,0.55,12.25,13.5,1.0) cade dentro il prior ma non è il suo centro.",
    },
    "primary_result": {
