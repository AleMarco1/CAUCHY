...
Nota sui fallback:
  n_gal target: ~573K (±40% → [300K, 800K])
  Cosmologie con n_gal < 300K dopo 20 tentativi → fallback log_Mmin=13.0
  Cosmologie con n_gal < 100 → fallback DM
...
Uso:
  python src/phase_b3_cal135.py [--n_sim 2000] [--seed 42] [--resume]
...
OUTPUT_FEATURES  = RESULTS_DIR / "phase_b3_cal135_features.npz"
OUTPUT_DIAG      = RESULTS_DIR / "phase_b3_cal135_diagnostics.json"
MANIFEST         = RESULTS_DIR / "phase_b3_cal135_manifest.json"
...
# Fallback HOD: log_Mmin=13.0 per cosmologie con pochi aloni massicci
HOD_FALLBACK_13 = HOD_CAL135.copy()
HOD_FALLBACK_13[0] = 13.0

# Finestra n_gal target (±40% di 573K)
N_GAL_LO       = 300_000
N_GAL_HI       = 800_000
N_GAL_TARGET   = 573_000
MAX_ATTEMPTS   = 20   # tentativi HOD prima del fallback log_Mmin=13.0
...
print(f"Finestra accettazione: [{N_GAL_LO:,}, {N_GAL_HI:,}]")
...
for sim_idx in range(args.n_sim):
    ...
    if pos_h is None or len(pos_h) < 50:
        ...
        # Tentativo 1: log_Mmin=13.5 deterministico
        pos_gal = populate_halos_hod(pos_h, mass_h, HOD_CAL135, rng_sim)
        n_gal   = len(pos_gal)
        hod_used = "13.5"

        # Se n_gal fuori finestra → fallback log_Mmin=13.0
        if n_gal < N_GAL_LO or n_gal > N_GAL_HI:
            rng_fb = np.random.default_rng(args.seed + sim_idx + 999999)
            pos_gal_fb = populate_halos_hod(pos_h, mass_h, HOD_FALLBACK_13, rng_fb)
            n_gal_fb   = len(pos_gal_fb)
            if n_gal_fb >= 100:
                pos_gal  = pos_gal_fb
                n_gal    = n_gal_fb
                hod_used = "13.0_fallback"
            # Se anche 13.0 fuori → usa comunque 13.5 (non scartare)
            # La variazione di densità tra cosmologie è il segnale che vogliamo
...
fallback_13 = [d for d in all_diags if d.get("hod_used") == "13.0_fallback"]
...
print(f"  Fallback log_Mmin=13.0:         {len(fallback_13)}")
...
np.savez(OUTPUT_FEATURES,
         ...
         methodology="HOD deterministico log_Mmin=13.5 calibrato densità BGS DR1 NGC")

diag_out = {
    "schema_version": "2.0",
    "task":           "B3_calibrated_log_Mmin_13.5",
    "timestamp":      datetime.now(timezone.utc).isoformat(),
    "motivation":     "Risposta BLOCKING 1 Reviewer R5-1 (2026-06-13): "
    ...
    "n_gal_window":   [N_GAL_LO, N_GAL_HI],
    "n_sims":         args.n_sim,
